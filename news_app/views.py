"""
View handlers and REST API viewsets for news_app[cite: 8].

Manages registration, article moderation, subscription workflows,
and REST endpoints for external clients[cite: 8].
"""
...
class IsJournalist(permissions.BasePermission):
    """Permission class restricting access strictly to authenticated users with the Journalist role[cite: 8]."""
    def has_permission(self, request, view):
        return request.user and request.user.is_authenticated and getattr(request.user, 'role', None) == 'journalist'


class IsEditor(permissions.BasePermission):
    """Permission class restricting access strictly to authenticated users with the Editor role[cite: 8]."""
    def has_permission(self, request, view):
...
class CustomUserCreationForm(UserCreationForm):
    """Form extending standard user registration with role selection and unique email validation[cite: 8]."""
    class Meta(UserCreationForm.Meta):
        model = CustomUser
        fields = UserCreationForm.Meta.fields + ('email', 'role',)

    def clean_email(self):
        """Validate that the provided email is non-empty and unique across accounts[cite: 8]."""
...
class ArticleForm(forms.ModelForm):
    """Form for journalists and editors to input article details[cite: 8]."""
    class Meta:
        model = Article
...
class NewsletterForm(forms.ModelForm):
    """Form to curate newsletters from approved articles[cite: 8]."""
    class Meta:
        model = Newsletter
        fields = ['title', 'description', 'articles']

    def __init__(self, *args, **kwargs):
        """Restrict article selection field to approved articles only[cite: 8]."""
        super().__init__(*args, **kwargs)
        self.fields['articles'].queryset = Article.objects.filter(approved=True)


class PublisherForm(forms.ModelForm):
    """Form to create and update Publisher organizations and staff assignments[cite: 8]."""
    class Meta:
        model = Publisher
...
class SubscriptionForm(forms.ModelForm):
    """Form enabling Reader users to subscribe to Publishers and Journalists[cite: 8]."""
    class Meta:
        model = CustomUser
    ...
    def __init__(self, *args, **kwargs):
        """Filter selectable journalists to active users possessing the Journalist role[cite: 8]."""
        super().__init__(*args, **kwargs)
...
def is_editor(user):
    """Check if the user is authenticated and holds the Editor role[cite: 8]."""
    return user.is_authenticated and user.role == 'editor'

def is_journalist(user):
    """Check if the user is authenticated and holds the Journalist role[cite: 8]."""
    return user.is_authenticated and user.role == 'journalist'

def is_journalist_or_editor(user):
    """Check if the user is authenticated and holds either Journalist or Editor role[cite: 8]."""
    return user.is_authenticated and user.role in ['journalist', 'editor']


# Views
def register_view(request):
    """Handle new user registration and redirect to role-specific dashboards[cite: 8]."""
...
def article_list_view(request):
    """Render public news feed displaying all approved articles ordered by creation date[cite: 8]."""
...
def manage_subscriptions_view(request):
    """Allow authenticated reader users to update publisher and journalist subscriptions[cite: 8]."""
...
def pending_articles_list(request):
    """Display pending articles review queue alongside live published articles for editors[cite: 8, 18]."""
...
def approve_article_action(request, article_id):
    """Approve a pending article submission and make it visible on public feeds[cite: 8]."""
...
def editor_edit_article_view(request, article_id):
    """Allow editors to modify existing articles directly[cite: 8]."""
    article = get_object_or_404(Article, id=article_id)
    if request.method == 'POST':
...
def editor_delete_article_view(request, article_id):
    """Allow editors to delete any article entry[cite: 8]."""
    article = get_object_or_404(Article, id=article_id)
...
def manage_publishers_view(request):
    """Display publisher listings and handle new publisher creation[cite: 8]."""
...
def create_article_view(request):
    """Enable journalists to compose and submit articles for editor review[cite: 8]."""
    if request.method == 'POST':
...
def my_articles_view(request):
    """Display articles authored by the currently logged-in journalist[cite: 8]."""
...
def edit_article_view(request, article_id):
    """Allow journalists to edit their own articles, resetting approval status upon save[cite: 8]."""
...
def delete_article_view(request, article_id):
    """Allow journalists to delete their own articles[cite: 8]."""
    article = get_object_or_404(Article, id=article_id, author=request.user)
    if request.method == 'POST':
        article.delete()
        return redirect('my_articles')
...
def newsletter_list_view(request):
    """Display public directory of all published newsletters[cite: 8]."""
...
def create_newsletter_view(request):
    """Allow journalists and editors to assemble and publish a newsletter[cite: 8]."""
    if request.method == 'POST':
...
def edit_newsletter_view(request, newsletter_id):
    """Allow editors (or the authoring journalist) to edit newsletter details[cite: 8]."""
...
def delete_newsletter_view(request, newsletter_id):
    """Allow editors (or the authoring journalist) to delete a newsletter[cite: 8]."""
    if request.user.role == 'editor':
        newsletter = get_object_or_404(Newsletter, id=newsletter_id)
    else:
        newsletter = get_object_or_404(Newsletter, id=newsletter_id, author=request.user)
...
class ArticleViewSet(viewsets.ModelViewSet):
    """
    REST API ViewSet enforcing role permissions across standard and custom endpoints[cite: 8, 18].
    """
    serializer_class = ArticleSerializer

    def get_queryset(self):
        """Retrieve approved articles for public REST listings[cite: 8]."""
        return Article.objects.filter(approved=True)

    def get_permissions(self):
        """Assign role-based permissions depending on requested action[cite: 8]."""
    ...
    def perform_create(self, serializer):
        """Attach authenticated user as author and force unapproved status upon submission[cite: 8]."""
        serializer.save(author=self.request.user, approved=False)

    @action(detail=True, methods=['post'], permission_classes=[IsEditor])
    def approve(self, request, pk=None):
        """API endpoint allowing editors to approve pending article submissions[cite: 8, 18]."""
    ...
    def subscribed(self, request):
        """API endpoint returning articles from publishers and journalists subscribed to by the reader[cite: 8]."""
...
def api_approved_log(request):
    """Internal webhook endpoint logging article approval events triggered by signals[cite: 5, 8]."""
